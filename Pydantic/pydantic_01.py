...
user = User(id=10, name="test_name", email="tname@example.com", age=40) # dto처럼 인자 값을 순서대로, key 값도 똑바로 작성해야 함
print("user:", user, user.id)

# dict keyword argument(kwargs)로 Pydantic Model 객체화
# 앞에 ** 기호를 붙여서 dict를 받아서 생성할수도 있음
...
json_dict = json.loads(json_string) # json 문자열을 dict로 변환 -> 그리고 위처럼 똑같이 동작함. 자바스크립트인줄;;
...
# AdvancedUser(advanced_level=9) -> Field required 에러 발생 -> 상속받은 필드까지 작성해야 함.
...
print(user_dump_01, type(user_dump_01)) # 타입이 dict

# json 문자열 기반으로 pydantic serialization
user_dump_02 = user.model_dump_json()
print(user_dump_02, type(user_dump_02)) # 타입이 str
